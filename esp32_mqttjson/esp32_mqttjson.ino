...
float battery_level = 100;    // ladeprosent for batteriet 0-100%
...
void setup() {
  ...
  client.setServer("172.20.10.5", 1883);
...
int battery_update() {  //this function updates battery_health to the flash-memory
...
void sendData() {
  ...
  if (millis() - jsonmillis > 1000) {
    ...
    jsonDoc1["traficcLight"] = trafficLight;
    String infoString;
...
void battery_drain() {  // continously drains battery by usage
  ...
  if ((millis() - drainMillis >= 1000) && (battery_level > 0) && (battery_health > 0) && (current_speed>1)) {  // if 1 second has passed, battery_level and battery_health is above 0
    ...
    battery_level = battery_level - (i - i / battery_health) / 5;  //battery level losing energy based on distance and the current state of the battery health
...
void callback(char* topic, byte* message, unsigned int length) {  // function that converts recived information
  ...
  if ((strcmp(topic, "startPoint") == 0)) {
    ...
    if (startPoint != laststartPoint) {
      sendDestination();  //sends the destination immediately through UART
